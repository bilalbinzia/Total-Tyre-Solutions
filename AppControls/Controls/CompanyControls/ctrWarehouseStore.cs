...
{
    public partial class ctrWarehouseStore : BaseControl
    {
        BindingSource WarehouseStoreBS;
        public ctrWarehouseStore()
        {
            InitializeComponent();
            WarehouseStoreBS = new BindingSource();

            this.Load += ctrWarehouseStore_Load;
        ...
        void btnAddRacks_Click(object sender, EventArgs e)
        {
            ...
            if (curRow != null)
            {
                if ((Convert.ToInt32(curRow["CompanyID"]) > 0) && (Convert.ToInt32(curRow["WarehouseID"]) > 0) && (Convert.ToInt32(curRow["ID"]) > 0))
                {
                    ctrRacks ctrRacks = new ctrRacks(Convert.ToInt32(curRow["CompanyID"]), Convert.ToInt32(curRow["WarehouseID"]), Convert.ToInt32(curRow["ID"]));
                    //----------------------------------------------------------------------//
        ...
        void cboWarehouse_SelectedIndexChanged(object sender, EventArgs e)
        {
            DefaultSettings();
        }
        void DefaultSettings()
        {
            if (frmStatus == currentStatus.Add)
            {
                ...
                if (curRow != null)
                {
                    if ((StaticInfo.CompanyID > 0) && (StaticInfo.WarehouseID > 0))
                    {
                        DataTable dt = dbClass.obj.getWarehouseStoreInfo(StaticInfo.StoreID);
                        if (dt.Rows.Count > 0)
                            curRow["CoCode"] = dt.Rows[0]["CoCode"];
                        if (dt.Rows.Count <= 0)
                            dt = dbClass.obj.getWarehouseInfo(StaticInfo.WarehouseID);
        ...
        void ctrWarehouseStore_Load(object sender, EventArgs e)
        {
        ...
        void LoadWarehouseStores()
        {
            ...
            if (curRow != null)
            {
                ...
                {
                    DataTable dt = dbClass.obj.FillStoreRacksList(Convert.ToInt32(curRow["ID"]));
