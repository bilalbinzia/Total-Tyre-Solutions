...
{
    public partial class ctrDashBoard3 : UserControl
    {
        static string CompAddress = "";
        ...
        {
            Helper dbClass = new Helper();
            string connectionString = dbClass.GetConnectionString();
            string queryString = "SELECT CoAddress FROM Company WHERE ID =" + StaticInfo.CompanyID;
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            CompAddress = (string)reader["CoAddress"];
                        }
                    }
                }
            }
            ...
            poweredByLabel.Text = "Powered and Developed By:"+Environment.NewLine+"Email: info@totaltiresolutions.com";
            
            timer1.Interval = 1000;
            timer1.Enabled = true;
            DigitalClock.TextAlign = ContentAlignment.MiddleRight;
            panel2.Padding = new Padding(0, 12, 20, 0);
            if (F12hr.Checked)
            {
            ...
            {
                Greetings.Text = "\"Good Morning, " + StaticInfo.EmployeeName + ".\"";
            }
            else if (now == Noon)
            {
                Greetings.Text = "\"Good Noon, " + StaticInfo.EmployeeName + ".\"";
            }
            else if (now > AfternoonStart && now < AfternoonEnd)
            {
                Greetings.Text = "\"Good Afternoon, " + StaticInfo.EmployeeName + ".\"";
            }
            else if (now > EveningStart && now < EveningEnd)
            {
                Greetings.Text = "\"Good Evening, " + StaticInfo.EmployeeName + ".\"" ;
            }
            else if (now > NightStart && now < NightEnd)
            {
                Greetings.Text = "\"Good Night," + StaticInfo.EmployeeName + ".\"";
            }
        ...
        private void btnWorkOrder_LinkClicked_1(object sender, LinkLabelLinkClickedEventArgs e)
        {
        ...
        private void Greetings_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
        ...
        private void Greetings_LinkClicked_1(object sender, LinkLabelLinkClickedEventArgs e)
        {

        }

        private void F12hr_CheckedChanged_1(object sender, EventArgs e)
        {
        ...
        private void openNotification_Click(object sender, EventArgs e)
        {
