...
namespace DBModule
{
    public sealed class dbClass
    {
        ...
        public bool SystemRegistration()
        {
            try
            {
                ...
                string cQry = "Select * from [dbo].[POSDetail] where [Com_port]='" + System32.SystemRegistration.cryptString + "'";
                SqlDataAdapter sDA = new SqlDataAdapter(cQry, this.connectionString);
                sDA.Fill(dt);
                if(dt.Rows.Count==0)
                {
        ...
        public DateTime GetExpiryDate()
        {
            ...
            {
                ExpiryDate = Convert.ToDateTime("2022-10-10");
                throw ex;
            }
            return ExpiryDate;
        }
        ...
        public int getNextVendorCode()
        {
            int Code = 0;
            try
            {
                string qry = "SELECT IsNull(MAX(Code),0)+1 FROM  Vendor";
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    sqlCon.Open();
                    SqlCommand sqlCmd = new SqlCommand(qry, sqlCon);
        ...
        public string FillCustomerListWithBalances()
        {
            string Qry = "Select m.[ID],m.[RegDate],m.[Code],m.[FirstName],m.[LastName],m.[IsCustomer],m.[IsCompany],m.[CompanyName]" +
                         ...
                         " Where m.CompanyID=" + StaticInfo.CompanyID;
        ...
        public DataTable getCustomerList()
        {
            ...
            string Qry = "SELECT [ID] ,[Code] ,[FirstName] ,[LastName],[CompanyName] ,[Address] ,[ContactPerson] ,[Email] ,[Phone1] FROM [dbo].[Customer] WHERE [CompanyID] = " +StaticInfo.CompanyID;
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = "SELECT * FROM [Customer] WHERE [ID] = " + CusID + "And CompanyID=" + StaticInfo.CompanyID;
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            try
            {
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = "SELECT TOP(1)* FROM [Customer] Where CompanyID=" + StaticInfo.CompanyID;
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            try
            {
        ...
        public DataTable FillCustomerList()
        {
            ...
            string Qry = "SELECT [ID] ,[Code] ,[FirstName] ,[LastName] ,[CompanyName],[Phone1] ,[Address] ,[ContactPerson] ,[Email],[IsCheckAccepted] " +
                         " FROM [dbo].[Customer] Where CompanyID=" + StaticInfo.CompanyID;
        ...
        public DataTable FillItemList()
        {
            ...
            string Qry = "SELECT itm.[ID]" +
                         ...
                         " ,itm.[Name]" +
                         " ,itm.[ItemGroupID]" +
                         " ,[InStock] = ISNULL((SELECT ISNULL(SUM([QtyOnHand]),0) FROM [dbo].[ItemStock] WHERE [ItemID] = Itm.ID GROUP BY [ItemID]),0)" +
                         " ,itm.[CatalogCost]" +
                         " ,itm.[FET]" +
                         " ,itm.[RetailPrice]" +
                         " ,itm.[WholeSalePrice]" +
        ...
        public DataTable FillWorkOrderHistory(int itmID)
        {
            DataTable dt = new DataTable();
            string Qry = "SELECT Mas.ID[ID],'WorkOrder'[Type],Ven.[CompanyName][Customer]," +
                         " itm.Name[Description],Mas.MID[Reference],wo.AddDate[Date],Mas.Qty[Qty]," +
                         " Mas.Price[Price],Mas.Price - Mas.Cost[Profit], Mas.FET[FET],emp.Initial[Rep]," +
                         " stor.CoName[Store] FROM dbo.WorkOrder wo Left Join " +
                         " dbo.WorkOrderDetail Mas on wo.ID = Mas.MID Left Join " +
                         " dbo.Item itm on Mas.ItemID = itm.ID Left Join " +
                         " dbo.Customer Ven on wo.CustomerID = Ven.ID Left Join" +
                         " dbo.Employee emp on wo.AddUserID = emp.ID Left Join" +
                         " dbo.WarehouseStore stor on wo.StoreID = stor.ID " +
                         " WHERE wo.[Total] > (select ISNULL(SUM(TotalReceivedAmount), 0)" +
                         " from dbo.CustomerReceipt WHERE WOID = WO.ID) AND Mas.ItemID = " + itmID + "And wo.CompanyID= " + StaticInfo.CompanyID + " order by Date Asc";
        ...
        {
            int companyId = StaticInfo.CompanyID;
            DataTable dt = new DataTable();
            string Qry = "SELECT [ID],[Type], [VendorCustomer] as Company, [Description] as Item, [Reference], Cast([Date] as date) Date, [Qty], [Price], [Profit], [FET], [Rep], [Store] FROM (" +

                        " SELECT Mas.ID[ID],'P/O'[Type],Ven.Name[VendorCustomer],itm.Name[Description],Mas.POID[Reference],det.AddDate[Date],det.PrevRcvd[Qty],det.Cost[Price], 0[Profit], det.FET[FET],emp.Initial[Rep],stor.CoName[Store] FROM dbo.PurchaseOrder Mas Left Join dbo.PurchaseOrderDetails det  on det.MID = Mas.ID" +
                        " Left Join dbo.Item itm on det.ItemID = itm.ID Left Join dbo.Vendor Ven on Mas.VendorID = Ven.ID Left Join dbo.Employee emp on Mas.AddUserID = emp.ID Left Join dbo.WarehouseStore stor on Mas.StoreID = stor.ID WHERE det.ItemID = " + itmID + " And Mas.CompanyID= " + companyId +

                        " UNION ALL " +

                        " SELECT Mas.ID[ID],'Sale'[Type],Ven.[CompanyName][VendorCustomer],itm.Name[Description],Mas.MID[Reference],cr.AddDate[Date],Mas.Qty[Qty],Mas.Price[Price],Mas.Price - Mas.Cost[Profit], Mas.FET[FET],emp.Initial[Rep],stor.CoName[Store] FROM dbo.CustomerReceipt cr Left Join" +
                        " dbo.WorkOrderDetail Mas on cr.WOID = Mas.MID Left Join dbo.Item itm on Mas.ItemID = itm.ID Left Join dbo.Customer Ven on cr.CustomerID = Ven.ID Left Join dbo.Employee emp on cr.AddUserID = emp.ID Left Join dbo.WarehouseStore stor on cr.StoreID = stor.ID WHERE  mas.ItemID = " + itmID + "  And Mas.CompanyID=cr.CompanyID And Mas.CompanyID= " + companyId +

                        " UNION ALL " +

                        " SELECT Mas.ID[ID],'Adjust'[Type],Emp.Name[VendorCustomer],itm.Name[Description],Mas.AdjNo[Reference],Mas.AdjustmentDate[Date],Mas.Qty[Qty],Mas.NewCost[Price],0[Profit], 0[FET],Emp.Initial[Rep],stor.CoName[Store] FROM " +
                        " dbo.InventoryAdjustment Mas Left Join dbo.Item itm on Mas.ItemID = itm.ID Left Join dbo.Employee Emp on Mas.AddUserID = Emp.ID Left Join dbo.WarehouseStore stor on Mas.StoreID = stor.ID WHERE  mas.ItemID = " + itmID + "  And Mas.CompanyID= " + companyId +

                        " UNION ALL " +

                        " SELECT Mas.ID[ID], 'Return'[Type],Ven.[CompanyName][VendorCustomer] ,itm.Name[Description],Mas.WorkOrderNegateNo[Reference],Mas.RegDate[Date],det.Qty[Qty],det.Price[Price],det.Price - det.Cost[Profit], det.FET[FET],emp.Initial[Rep],stor.CoName[Store] FROM dbo.CustomerPayment cp Left Join" +
                        " dbo.WorkOrderNegate Mas on cp.WONID = Mas.ID  Left join dbo.WorkOrderNegateDetail det on det.MID = Mas.ID Left Join dbo.Item itm on det.ItemID = itm.ID Left Join dbo.Customer Ven on Mas.CustomerID = Ven.ID Left Join dbo.Employee emp on Mas.SaleRepID = emp.ID Left Join dbo.WarehouseStore stor on Mas.StoreID = stor.ID WHERE det.ItemID = " + itmID + " And Mas.CompanyID=cp.CompanyID And Mas.CompanyID= " + companyId + ") tbl" +
                        " order by Date Asc";
        ...
        public DataTable getItemsForGridByCustomerID(int CusID, string PriceColumn)
        {
            ...
            string Qry = @" SELECT 
                             ...
                             ( 
	                            ...
	                            itm.Name [Name] ,
	                            [Available] = (SELECT ISNULL([Qty],0) FROM [dbo].[ItemStock] WHERE ItemID = itm.[ID]) ,
	                            [Qty] = 1 ,itm.CatalogCost ,
	                            [Price] = "+ PriceColumn + @" ,
	                            [Amount] = " + PriceColumn + @",
	                            [Total] = " + PriceColumn + @",
	                            [IsOptional] = 0 ,
	                            [Hours] = 0 ,
	                            itm.IsTemporaryDiscount [IsTemporaryDiscount],
	                            itm.TemporaryDiscountDateFrom [TemporaryDiscountDateFrom],
	                            itm.TemporaryDiscountDateTo [TemporaryDiscountDateTo],
	                            itm.TemporaryDiscountedPriceA [TemporaryDiscountedPriceA],
	                            itm.TemporaryDiscountedPriceB [TemporaryDiscountedPriceB],
	                            itm.TemporaryDiscountedPriceC [TemporaryDiscountedPriceC],
	                            itm.IsSpiffsTemporary [IsSpiffsTemporary],
	                            itm.SpiffsDateFrom [SpiffsDateFrom],
	                            itm.SpiffsDateTo [SpiffsDateTo],
	                            itm.SpiffsDollarAmount [SpiffsDollarAmount], 
	                            itm.[IsDiscountable] ,
	                            [PartDisPer] = CASE itm.[IsDiscountable] WHEN 1 THEN (SELECT ISNULL(PartDisPer,0) FROM dbo.Customer WHERE ID = 29698) ELSE 0 END ,
	                            [PartDis] = CASE itm.[IsDiscountable] WHEN 1 THEN CAST(ROUND(itm.PriceD * ((SELECT ISNULL(PartDisPer,0) FROM dbo.Customer WHERE ID = 29698)/100),2) as numeric(36,2)) ELSE 0 END ,
	                            [FET] = CASE itm.[IsUseFET] WHEN 1 THEN ISNULL(itm.[FET],0) else 0 end ,
	                            itm.[IsTaxable] ,
	                            [SaleTaxRate] = 0 ,
	                            [PartTax] = 0 ,
	                            [MarginPer] = [PriceDPercent] ,
	                            [MarginAmount] =  0 ,
	                            ISNULL(itm.[IsRepComm],0) [IsRepComm] ,
	                            ISNULL(itm.[IsMechComm],0) [IsMechComm] FROM dbo.Item itm ) Itm";
        ...
        public DataTable getItemForPO(int ItemID)
        {
            ...
            string Qry = "SELECT [Itm].* ,tbl1.*" +
                         " ,[IType].[Name] [ItemType], 0 [IsUseGroupMargin], 0 [IsUpdateCost]" +
                         " , 0 [QtyOrdrd] ,0 [PrevOrdrd] ,0 [QtyRcvd] ,0 [PrevRcvd] ,0 [QtyBilled] ,0 [PrevBilled],0 [ReOrderQty], IsNull(ItmStock.Qty,0) [AvailableQty], IsNull(ItmStock.QtyOnHand,0) [OnHandQty]" +
                         " FROM [dbo].[Item] [Itm] Left Join [dbo].[ItemStock] [ItmStock] ON [ItmStock].[ItemID] = [Itm].[ID] " +
                         " Left Join [dbo].[ItemType] [IType] ON [Itm].[ItemTypeID] = [IType].ID" +
                         " Cross Join ( Select (SUM(ISNULL([PODetail].[QtyOrdrd],0) + ISNULL([PODetail].[PrevOrdrd],0)) - SUM(ISNULL([PODetail].[QtyRcvd],0) + ISNULL([PODetail].[PrevRcvd],0))) [OnOrderQty] FROM [dbo].[PurchaseOrderDetails] [PODetail] where ItemID=" + ItemID + " ) tbl1" +
                         " WHERE [Itm].[ID] = " + ItemID;
        ...
        public void UpdateItemCatalogCost(DataTable dt)
        {
            SqlConnection SqlCnn = new SqlConnection(this.connectionString);
            try
            {
        ...
        public DataTable FillAccountList()
        {
            ...
            string Qry = "select  [A].[ID],[A].[AccID],[A].[AccTypeID] ,[A].[AccName] ,[A1].[AccName][Acc.Type],[A].[Active] from Account A left join Account A1 on A.AccTypeID=A1.AccID order by A1.AccName";
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = "select  [A].[ID],[A].[AccID],[A].[AccTypeID] ,[A].[AccName] ,[A].[AccNo] ,[A1].[AccName],[A].[Active] from Account A left join Account A1 on A.AccTypeID=A1.AccID where A.AccID= " + AccID + "order by A1.AccName";
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = " SELECT [ID],[AccID],[DocNo] FROM [dbo].[Account] WHERE [ID] = " + ID;
        ...
        public DataTable GetSaleAccountsList(int saleID,int taxID )
        {
            DataTable dt = new DataTable();
            string qry = "SELECT * FROM [dbo].[Account] WHERE [AccTypeID] ="+ saleID + " OR [AccTypeID] =" + taxID + "";
            using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
            {
                sqlCon.Open();
                SqlCommand sqlCmd = new SqlCommand(qry, sqlCon);
        ...
        {
            int EmployeeID = 0;
            string Qry = "SELECT ID FROM [Employee] WHERE [Name] = '" + UserName + "'";
        ...
        {
            string Qry = "SELECT [ID], [Name], [Initial], [Phone1], [Phone2], [Active]" +
                         " FROM [dbo].[Employee]" +
                         " WHERE [Active] = 1" +
                         " Order by [Name]";
        ...
        public DataTable FillPurchaseOrdersByVendorID(DataTable dt, int VendorID)
        {
            ...
            string Qry = "SELECT [PO].*" +
                         ...
                         " Left Join [dbo].[Employee] [emp] ON [PO].[AddUserID] = [emp].ID" +
                         " Left Join [dbo].[WarehouseStore] [str] ON [PO].[StoreID] = [str].ID" +
                         " WHERE [PO].[VendorID] = " + VendorID + " And [PO].[CompanyID]= " + StaticInfo.CompanyID + " Order by [PO].ID desc";
        ...
        public DataTable FillVendorBillsByVendorIDforPayments(DataTable dt, int VendorID)
        {
            ...
            string Qry = "SELECT [VB].[ID] ,[VB].[BillID] ,[VB].[VendorID] ,[VB].[BillDate] ,[VB].[DueDate] ,[VB].[InvoiceNo] [Reference] ,[VB].[BillNotes] [Notes] " +
                         ...
                         " ,[TotalPaidAmount] = (SELECT isnull(SUM(ISNULL(PaidAmount, 0)), 0) FROM[dbo].[VendorPayment] WHERE[BillID] = [VB].[BillID] AND Active=1)" +
                         " ,[Payable] = isnull([VB].BillTotalAmount,0) -(SELECT isnull(SUM(ISNULL([BillDiscount], 0)),0) FROM[dbo].[VendorPayment] WHERE[BillID] = [VB].[BillID] AND Active=1) -(SELECT isnull(SUM(ISNULL(PaidAmount, 0)), 0) FROM[dbo].[VendorPayment] WHERE[BillID] = [VB].[BillID] AND Active=1)" +
                         " ,[Discount] = 0" +
                         " ,0.00[To Pay]" +
                         " ,[Balance] = isnull([VB].BillTotalAmount,0) -(SELECT isnull(SUM(ISNULL([BillDiscount], 0)),0) FROM[dbo].[VendorPayment] WHERE[BillID] = [VB].[BillID] AND Active=1) -(SELECT isnull(SUM(ISNULL(PaidAmount, 0)), 0) FROM[dbo].[VendorPayment] WHERE[BillID] = [VB].[BillID] AND Active=1)" +
                         " ,[Type] = case when(isnull([VB].BillTotalAmount, 0) - (SELECT isnull(SUM(ISNULL([BillDiscount], 0)),0) FROM[dbo].[VendorPayment] WHERE[BillID] = [VB].[BillID] AND Active=1) -(SELECT isnull(SUM(ISNULL(PaidAmount, 0)), 0) FROM[dbo].[VendorPayment] WHERE[BillID] = [VB].[BillID] AND Active=1)) > 0 then 'Bill' else 'Payment' end" +
                         ...
                         " Left Join[dbo].[Vendor] [Frgt] ON[VB].[BillPayFreightToID] = [Frgt].ID" +
                         " Left Join[dbo].[Terms] [trm] ON ven.TermsID = trm.ID" +
                         " WHERE [VB].[VendorID] = " + VendorID + " And [VB].[CompanyID] = " + StaticInfo.CompanyID + "Order by[VB].ID desc";
        ...
        public DataTable FillVendorPaymentHistoryPaymentByVendorID(int VenID)
        {
            ...
            string Qry = "Select VendorPaymentHistory.ID, VendorPaymentHistory.PaymentID, VendorPaymentHistory.VendorID, VendorPaymentHistory.TrnsDate, VendorPaymentHistory.InvoiceNo, VendorPaymentHistory.PaidAmount, VendorPaymentHistory.CompanyID," +
                        ...
                        " WHERE VendorPaymentHistory.VendorID = " + VenID + " And VendorPaymentHistory.CompanyID = " + StaticInfo.CompanyID + " AND VendorPaymentHistory.Active=1  Order by VendorPaymentHistory.PaymentID DESC";
        ...
        public DataTable FillVendorPaymentHistoryVoidByVendorID(int VenID)
        {
            ...
            string Qry = "Select VendorPaymentHistory.ID, VendorPaymentHistory.PaymentID, VendorPaymentHistory.VendorID, VendorPaymentHistory.TrnsDate, VendorPaymentHistory.InvoiceNo, VendorPaymentHistory.PaidAmount, VendorPaymentHistory.CompanyID," +
                        ...
                        " Where CHARINDEX(',' + CAST(VendorBill.BillID as varchar(50)) + ',', VendorPaymentHistory.BillIDs) > 0 FOR XML PATH('')), 1, 1, '')) As BillIDs" +
                        " From VendorPaymentHistory Left Outer Join WarehouseStore ON VendorPaymentHistory.StoreID = WarehouseStore.ID" +
                        " WHERE VendorPaymentHistory.VendorID = " + VenID + " And VendorPaymentHistory.CompanyID = " + StaticInfo.CompanyID + " AND VendorPaymentHistory.Active=0  Order by VendorPaymentHistory.PaymentID DESC";
        ...
        public DataTable FillVendorPaymentHistoryAllByVendorID(int VenID)
        {
            ...
            string Qry = "Select VendorPaymentHistory.ID, VendorPaymentHistory.PaymentID, VendorPaymentHistory.VendorID, VendorPaymentHistory.TrnsDate, VendorPaymentHistory.InvoiceNo, VendorPaymentHistory.PaidAmount, VendorPaymentHistory.CompanyID," +
                        ...
                        " Where CHARINDEX(',' + CAST(VendorBill.BillID as varchar(50)) + ',', VendorPaymentHistory.BillIDs) > 0 FOR XML PATH('')), 1, 1, '')) As BillIDs" +
                        " From VendorPaymentHistory Left Outer Join WarehouseStore ON VendorPaymentHistory.StoreID = WarehouseStore.ID" +
                        " WHERE VendorPaymentHistory.VendorID = " + VenID + " And VendorPaymentHistory.CompanyID = " + StaticInfo.CompanyID + " Order by VendorPaymentHistory.PaymentID DESC";
        ...
        public int AddCustomerCreditHistory(int RecepitID, int InvoiceID, decimal PaidAmount, int CustomerID)
        {
            int NextID = 0;

            using (SqlConnection conn = new SqlConnection(this.connectionString))
            using (SqlCommand cmd = new SqlCommand("dbo.AddCustomerCreditHistory", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public void UpdateCustomerReceiptCredit(string InvoiceIDs)
        {
            try
            {

                string Qry = "UPDATE [dbo].[CustomerReceipt]" +
                             " SET [IsCredit] = 0" +
                             " WHERE CHARINDEX(',' + cast(ID as varchar(20)) + ',' , '" + InvoiceIDs + "') > 0";
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    sqlCon.Open();
                    SqlCommand sqlCmd = new SqlCommand(Qry, sqlCon);
        ...
        public int AddVendorPayment(int PaymentID, int VendorID, int BillID, string InvoiceNo, DateTime TrnsDate, string TrnsNotes, string TrnsType, decimal BillAmount, decimal BillDiscount, decimal PaidAmount, decimal BillBalance, bool IsPaid)
        {
            ...
            using (SqlConnection conn = new SqlConnection(this.connectionString))
            {
                ...
                try
                {
                    using (SqlCommand cmd = new SqlCommand("dbo.AddVendorPayment", conn,transaction))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable getVendorPaymentByVendorIDAndPaymentID(int VendorID, int PaymentID)
        {
            DataTable dataTable = new DataTable();
            string Qry = "SELECT * FROM VendorPayment Where VendorID =" + VendorID +
                "AND PaymentID =" + @PaymentID;
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
        ...
        public DataTable FillLatestCustomerWorkOrders()
        {
            ...
            string Qry = " SELECT ID ,[TransNo] ,[Date] ,[Type] ,[SaleRepID] ,[Paid by] ,[CustomerID] ,[Rep] ,[PONo] ,[Mileage] ,[VehicleID] ,[Vehicle] ,[Open Amt] ,[Total Amt] ,[MechID] ,[Mech]" +
                         ...
                         " where WOID = WO.ID) And WO.CompanyID = " + StaticInfo.CompanyID + " ) tbl" +
                         " UNION ALL" +
                         ...
                         " where WONID = WO.ID) And WO.CompanyID = " + StaticInfo.CompanyID + ") tbl1";
        ...
        public DataTable FillSelectedCustomerWorkOrders(int CustomerID)
        {
            ...
            string Qry = " SELECT ID ,[TransNo] ,[Date] ,[InvoiceNo] ,[Type] ,[SaleRepID] ,[Paid by] ,[CustomerID] ,[Rep] ,[PONo] ,[Mileage] ,[VehicleID] ,[Vehicle] ,[Open Amt] ,[Total Amt] ,[MechID] ,[Mech]" +
                         " ,[Status] = CASE [Type] WHEN 'Invoice' THEN 'Completed'  ELSE [Status] END ,[Description] ,[SaleCategoryID] ,[Category] ,[ShipViaID] ,[ShipVia] ,[Phone] ,[StoreID] ,[Store]" +
                         " ,[ctrName] = CASE [Type] WHEN 'Invoice' THEN 'ctrCustomerReceipt' ELSE 'ctrWorkOrder' END" +
                         " ,[IsNegated], [IsLocked]" +
                         " FROM" +
                         " (SELECT WO.[ID]" +
                         " ,[WO].[WorkOrderNo] [TransNo] ,WO.[RegDate] [Date], CR.[InvoiceNo]" +
                         " ,[Type] = CASE WO.[IsQutation] WHEN 1 THEN 'Qutation' ELSE CASE WO.[IsWorkOrder] When 0 Then 'Adjust' WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt" +
                         " where WOID = WO.ID) THEN 'WorkOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID)" +
                         " THEN 'Invoice' END ELSE CASE WO.[IsCustomerOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'CusOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'Invoice' END  ELSE '' END END END ,[WO].[WorkOrderNo] ,WO.[SaleRepID]" +
                         " ,cus.CompanyName [Paid by] ,WO.[CustomerID] ,emp.Initial [Rep] ,CR.TrnsNotes[PONo] ,WO.[Mileage] ,WO.[VehicleID]" +
                         " ,[Vehicle] = vy.Name + ' ' + vmk.Name + ' ' +vmd.Name" +
                         " ,[Open Amt] = WO.[Total] - (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID)" +
                         " ,WO.[Total] [Total Amt]" +
                         " ,WO.[MechID] ,emp1.Initial [Mech] ,WO.[Status] ,WO.Notes [Description] ,WO.[SaleCategoryID] ,sCat.Name [Category] ,WO.[ShipViaID] ,sVia.Name [ShipVia] ,cus.Phone1 [Phone] ,WO.[StoreID] ,wStr.CoName [Store],WO.[IsNegated], WO.[IsLocked]" +
                         " FROM [dbo].[WorkOrder] [WO]  Left Join dbo.CustomerReceipt CR on WO.ID = CR.WOID  Left Join dbo.Customer cus on wo.CustomerID = cus.ID Left Join dbo.Employee emp on WO.[SaleRepID] = emp.ID Left Join dbo.Vehicle veh on wo.VehicleID = veh.ID Left Join dbo.VehicleYear vy on veh.VehicleYearID = vy.ID Left Join dbo.VehicleMake vmk on veh.VehicleMakeID = vmk.ID Left Join dbo.VehicleModel vmd on veh.VehicleModelID = vmd.ID Left Join dbo.Employee emp1 on WO.[MechID] = emp1.ID Left Join dbo.SaleTaxCategory sCat on WO.[SaleCategoryID] = sCat.ID Left Join dbo.ShipVia sVia on WO.[ShipViaID] = sVia.ID Left Join dbo.WarehouseStore wStr on WO.StoreID = wStr.ID" +
                         " WHERE WO.CustomerID = " + CustomerID + "And WO.CompanyID=" + StaticInfo.CompanyID + " ) tbl" +
                         " UNION ALL" +
                         " SELECT ID ,[TransNo] ,[Date] ,[InvoiceNo] ,[Type]  ,[SaleRepID] ,[Paid by] ,[CustomerID] ,[Rep] ,[PONo] ,[Mileage] ,[VehicleID] ,[Vehicle] ,[Open Amt] ,[Total Amt] ,[MechID] ,[Mech]" +
                         " ,[Status] = CASE [Type] WHEN 'Negate' THEN 'Completed' ELSE [Status] END ,[Description] ,[SaleCategoryID] ,[Category] ,[ShipViaID] ,[ShipVia] ,[Phone] ,[StoreID] ,[Store]" +
                         " ,[ctrName] = CASE [Type] WHEN 'Negate' THEN 'ctrCustomerPayment' ELSE 'ctrWorkOrderNegate' END" +
                         " ,[IsNegated], [IsLocked]" +
                         " FROM" +
                         " (SELECT WO.[ID]" +
                         " ,[WO].[WorkOrderNegateNo] [TransNo] ,WO.[RegDate] [Date], CP.[InvoiceNo]" +
                         " ,[Type] = CASE WO.[IsQutation] WHEN 1 THEN 'Qutation' ELSE CASE WO.[IsWorkOrderNegate] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerPayment" +
                         " where WONID = WO.ID) THEN 'WorkOrderNegate' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerPayment where WONID = WO.ID)" +
                         " THEN 'Negate' END ELSE CASE WO.[IsCustomerOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'CusOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'Negate' END  ELSE '' END END END ,[WO].[WorkOrderNegateNo] ,WO.[SaleRepID]" +
                         ...
                         " ,(WO.[Total]*-1) [Total Amt]" +
                         " ,WO.[MechID] ,emp1.Initial [Mech] ,WO.[Status] ,WO.Notes [Description] ,WO.[SaleCategoryID] ,sCat.Name [Category] ,WO.[ShipViaID] ,sVia.Name [ShipVia] ,cus.Phone1 [Phone] ,WO.[StoreID] ,wStr.CoName [Store],[IsNegated] = 0, WO.[IsLocked]" +
                         " FROM [dbo].[WorkOrderNegate] [WO]  Left Join dbo.CustomerPayment CP on WO.ID = CP.WONID   Left Join dbo.Customer cus on wo.CustomerID = cus.ID Left Join dbo.Employee emp on WO.[SaleRepID] = emp.ID Left Join dbo.Vehicle veh on wo.VehicleID = veh.ID Left Join dbo.VehicleYear vy on veh.VehicleYearID = vy.ID Left Join dbo.VehicleMake vmk on veh.VehicleMakeID = vmk.ID Left Join dbo.VehicleModel vmd on veh.VehicleModelID = vmd.ID Left Join dbo.Employee emp1 on WO.[MechID] = emp1.ID Left Join dbo.SaleTaxCategory sCat on WO.[SaleCategoryID] = sCat.ID Left Join dbo.ShipVia sVia on WO.[ShipViaID] = sVia.ID Left Join dbo.WarehouseStore wStr on WO.StoreID = wStr.ID" +
                         " WHERE WO.CustomerID = " + CustomerID + "And WO.CompanyID=" + StaticInfo.CompanyID + " ) tbl1 " +
                         "  UNION ALL" +
                         " SELECT ID ,[TransNo] ,[Date] ,[InvoiceNo] ,[Type]  ,null [SaleRepID] ,[Paid by] ,[CustomerID] ,[Rep] ,[PONo],null [Mileage] , null [VehicleID] , null[Vehicle]  ,[Open Amt] ,[Total Amt] " +
                         " ,null[MechID] , null[Mech] ,[Status] = 'Completed' ,[Description] ,null[SaleCategoryID] ,null[Category] ,null[ShipViaID] ,null[ShipVia] ,[Phone],[StoreID] ,[Store]" +
                         " ,[ctrName] = CASE [Type] WHEN 'Payment' THEN 'ctrCustomerReceipt'ELSE 'ctrWorkOrder' END " +
                         " ,[IsNegated], [IsLocked]" +
                         " FROM" +
                         " (SELECT CR.[ReceiptID][ID]" +
                         " ,[CR].[ReceiptID] [TransNo] ,CR.[TrnsDate] [Date], CR.[InvoiceNo]" +
                         " ,[Type] = 'Payment' " +
                         " ,[CR].[ReceiptID] ,'' as SaleRepID" +
                         " ,cus.CompanyName [Paid by] ,CR.[CustomerID] ,emp.Initial [Rep]" +
                         " ,CR.TrnsNotes[PONo]" +
                         " ,[Open Amt] = 0" +
                         " ,(CR.[TotalReceivedAmount]) [Total Amt]" +
                         " ,CR.TrnsNotes [Description] " +
                         " ,cus.Phone1 [Phone] " +
                         " ,wStr.CoName [Store] ,wStr.ID [StoreID] ,[IsNegated] = 0, CR.[IsLocked]" +
                         " FROM dbo.CustomerReceipt CR Left Join dbo.Customer cus on CR.CustomerID = cus.ID Left Join dbo.Employee emp on CR.[AddUserID] = emp.ID " +
                         " Left Join dbo.WarehouseStore wStr on CR.StoreID = wStr.ID" +
                         " WHERE WOID is null and  CR.CustomerID = " + CustomerID + " And CR.CompanyID=" + StaticInfo.CompanyID + " ) tbl ";
        ...
        public DataTable FillSelectedCustomerCreditedInvoice(int CustomerID)
        {
            DataTable dataTable = new DataTable();

            string Qry = " SELECT ID ,[TransNo] ,[Date] ,[InvoiceID] ,[InvoiceNo] ,[Type] ,[IsCredit] ,[ChgOnAccount] ,[SaleRepID] ,[Paid by] ,[CustomerID] ,[Rep] ,[PONo] ,[Mileage] ,[VehicleID] ,[Vehicle] ,[Open Amt] ,[Total Amt] ,[MechID] ,[Mech]" +
                         " ,[Status] = CASE [Type] WHEN 'Invoice' THEN 'Completed'  ELSE [Status] END ,[Description] ,[SaleCategoryID] ,[Category] ,[ShipViaID] ,[ShipVia] ,[Phone] ,[StoreID] ,[Store]" +
                         " ,[ctrName] = CASE [Type] WHEN 'Invoice' THEN 'ctrCustomerReceipt' ELSE 'ctrWorkOrder' END" +
                         " ,[IsNegated], [IsLocked]" +
                         " FROM" +
                         " (SELECT WO.[ID]" +
                         " ,[WO].[WorkOrderNo] [TransNo] ,WO.[RegDate] [Date] ,CR.[ID][InvoiceID] ,CR.[InvoiceNo], Isnull(CR.[IsCredit],0) as IsCredit " +
                         //" ,Isnull(CR.[ChgOnAccount],0) as ChgOnAccount" +
                         " ,[ChgOnAccount] = Isnull(CR.[ChgOnAccount],0) - (Select ISNULL(SUM(PaidAmount),0) From dbo.CustomerCreditHistory Where InvoiceID = CR.ID)" +
                         " ,[Type] = CASE WO.[IsQutation] WHEN 1 THEN 'Qutation' ELSE CASE WO.[IsWorkOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt" +
                         " where WOID = WO.ID) THEN 'WorkOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID)" +
                         " THEN 'Invoice' END ELSE CASE WO.[IsCustomerOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'CusOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'Invoice' END  ELSE '' END END END ,[WO].[WorkOrderNo] ,WO.[SaleRepID]" +
                         " ,cus.CompanyName [Paid by] ,WO.[CustomerID] ,emp.Initial [Rep] ,WO.[PONo] ,WO.[Mileage] ,WO.[VehicleID]" +
                         ...
                         " ,WO.[MechID] ,emp1.Initial [Mech] ,WO.[Status] ,WO.Notes [Description] ,WO.[SaleCategoryID] ,sCat.Name [Category] ,WO.[ShipViaID] ,sVia.Name [ShipVia] ,cus.Phone1 [Phone] ,WO.[StoreID] ,wStr.CoName [Store],WO.[IsNegated], WO.[IsLocked]" +
                         " FROM [dbo].[WorkOrder] [WO]  Left Join dbo.CustomerReceipt CR on WO.ID = CR.WOID  Left Join dbo.Customer cus on wo.CustomerID = cus.ID Left Join dbo.Employee emp on WO.[SaleRepID] = emp.ID Left Join dbo.Vehicle veh on wo.VehicleID = veh.ID Left Join dbo.VehicleYear vy on veh.VehicleYearID = vy.ID Left Join dbo.VehicleMake vmk on veh.VehicleMakeID = vmk.ID Left Join dbo.VehicleModel vmd on veh.VehicleModelID = vmd.ID Left Join dbo.Employee emp1 on WO.[MechID] = emp1.ID Left Join dbo.SaleTaxCategory sCat on WO.[SaleCategoryID] = sCat.ID Left Join dbo.ShipVia sVia on WO.[ShipViaID] = sVia.ID Left Join dbo.WarehouseStore wStr on WO.StoreID = wStr.ID" +
                         " WHERE CR.IsCredit=1 And WO.CustomerID = " + CustomerID + " And WO.CompanyID = " + StaticInfo.CompanyID + " ) tbl";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        "ChgOnAccount,PayByCash,PaybyCheck,CheckNo,LicNo,PayByDeposit,PayByVisa,PayByMC,PayByAMEX,PayByATM,PayByGY,PayByDSCVR,TotalReceivedAmount,Active,AddDate,AddUserID,ModifyUserID,ModifyDate,Comments,IsLocked,DocNo,Remarks,CompanyID,WarehouseID,StoreID) " +
                                             ...
                                             "@ChgOnAccount,@PayByCash,@PaybyCheck,@CheckNo,@LicNo,@PayByDeposit,@PayByVisa,@PayByMC,@PayByAMEX,@PayByATM,@PayByGY,@PayByDSCVR,@TotalReceivedAmount,@Active,@AddDate,@AddUserID,@ModifyUserID,@ModifyDate,@Comments,@IsLocked,@DocNo,@Remarks,@CompanyID,@WarehouseID,@StoreID)", SqlCnn, transaction);
        ...
        public DataTable GetAccountsList(int AccountTypeID, int taxID)
        {
            DataTable dt = new DataTable();
            string Qry = "";
            if (taxID != 0)
            {
                Qry = "SELECT * FROM [dbo].[Account] WHERE [AccTypeID] =" + AccountTypeID + " OR [AccTypeID] =" + taxID + "";
            }
            else
            {
                Qry = "SELECT * FROM [dbo].[Account] WHERE [AccTypeID] =" + AccountTypeID + "";
            }
            using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
            {
                sqlCon.Open();
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = @"select  row_number() over(order by CAST(cr.TrnsDate AS DATE))AS [Rep No.]," +
                " CAST(cr.TrnsDate AS DATE) as [Report Date],CAST(cr.TrnsDate AS DATE) as [From Date]," +
                " CAST(cr.TrnsDate AS DATE) as [To Date]," +
                " CONVERT(varchar, sum(cr.TotalReceivedAmount)-(select ISNULL(Sum(TotalReceivedAmount) ,0) from CustomerPayment where CAST(TrnsDate AS DATE)= cr.TrnsDate)) as [Total Sale]," +
                " CONVERT(varchar, sum(cr.PaybyCheck)-(select ISNULL(Sum(PaybyCheck) ,0) from CustomerPayment where CAST(TrnsDate AS DATE)= cr.TrnsDate)) as [Cheque Sale]," +
                " CONVERT(varchar, sum(cr.ChgOnAccount)-(select ISNULL(Sum(ChgOnAccount) ,0) from CustomerPayment where CAST(TrnsDate AS DATE)= cr.TrnsDate)) as [Account Sale]," +
                " cr.IsLocked as Posted from [WorkOrder] wo Inner join CustomerReceipt cr on wo.ID = cr.WOID " +
                " Where cr.CompanyID = " + StaticInfo.CompanyID +
                " group by CAST(cr.TrnsDate AS DATE),cr.IsLocked order by row_number()over(order by CAST(cr.TrnsDate AS DATE)) Desc";
        ...
        public DataTable GetDailyTransactionsListbyDate(string date, int AccountID)
        {
            DataTable dataTable = new DataTable();
            string Qry = "";
            if (AccountID == 4)
            {
                Qry = @"select av.vNo as Tran#,FirstName + ' '+ LastName as Customer,
                           av.InvoiceID,
                           av.vDate as [Date],
                           av.TotalAmount,
						   av.AccountID,
                           av.AmountIn,
                           av.WOID
                           from Customer c inner join AccountVoucher av on c.ID=av.CustomerID
                           where av.AccountID=" + AccountID + " AND CAST(av.vDate as DATE)='" + date + "'";
            }
            else 
            {
                Qry = @"select  av.InvoiceID,
                           Name as Vendor,
						   av.vNo as Tran#,
                           av.vDate as [Date],
                           av.TotalAmount,
                           ...
                           av.AmountIn,
                           NULL AS WOID
                           from Vendor v inner join AccountVoucher av on v.ID=av.VendorID
                           where av.AccountID=" + AccountID + " AND CAST(av.vDate as DATE)='" + date + "'";
            }
        ...
        {
            DataTable dataTable = new DataTable();
            //string Qry = "Select cr.ID AS [Trans#],CR.TrnsDate as [Transaction Date],WOID AS WO#,TrnsNotes as PaidBy,(SELECT name from Employee WHERE ID=WO.AddUserID)as Rep,wo.Cost,wo.Price,CONVERT(varchar, wo.Profit)as Profit,CONVERT(varchar, wo.MarginPer) as Margin,CONVERT(varchar, CR.TotalReceivedAmount) AS TotalAmount FROM CustomerReceipt CR inner join WorkOrderDetail wo on wo.MID = CR.WOID Where CR.CompanyID=" + StaticInfo.CompanyID + " And CAST(CR.AddDate as DATE)='" + date + "'";
            //string Qry = "Select cr.ID AS [Trans#],CR.TrnsDate as [Transaction Date],WOID AS WO#,TrnsNotes as PaidBy,(SELECT name from Employee WHERE ID=WO.AddUserID)as Rep,wo.Cost,wo.Price,CONVERT(varchar, wo.Profit)as Profit,CONVERT(varchar, wo.MarginPer) as Margin,CONVERT(varchar, CR.TotalReceivedAmount) AS TotalAmount FROM CustomerReceipt CR inner join WorkOrderDetail wo on wo.MID = CR.WOID Where CR.CompanyID=" + StaticInfo.CompanyID + " And CAST(CR.AddDate as DATE)='" + date + "'";
            string Qry = "Select cr.ID AS[Trans#],CR.TrnsDate as [Transaction Date],WOID AS WO#,TrnsNotes as PaidBy,(SELECT name from Employee WHERE ID=WO.AddUserID)as Rep, CONVERT(varchar,wo.Total) as Amount,CONVERT(varchar, wo.TotalProfit)as Profit,CONVERT(varchar, CR.TotalReceivedAmount) AS TotalAmount FROM CustomerReceipt CR left join WorkOrder wo on wo.ID = CR.WOID Where CR.CompanyID=" + StaticInfo.CompanyID + " And CAST(CR.AddDate as DATE)='" + date + "'";
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = "Select wod.MID as ID,wod.ItemID,wod.FeeID,wod.laborID,CAST(wod.AddDate as DATE) as [Date],cr.TrnsNotes as [Paid By],WOD.Qty,wod.Ctype,wod.[Hours],wod.Price,wod.Amount,wod.MarginPer as Margin,wod.FET,wod.Tax,wod.DiscAmount as Discount,wod.Total from WorkOrderDetail wod inner join CustomerReceipt cr on cr.WOID=wod.MID where wod.MID='" + ID + "' AND cr.CompanyID=" + StaticInfo.CompanyID + " AND CAST(cr.TrnsDate as DATE)='" + date + "'";
            ...
            dataTable.Columns["Cost"].DefaultValue = "0";
            ...
            for (int i = 0; i < dataTable.Rows.Count; i++)
            {
                ...
                {
                    DataTable dt = new DataTable();
                    string sQry = "Select Catalog,Name, IsNull(LastCost,0) as Cost,(Select Name from ItemGroup where ID=ItemGroupID)as [Group] from Item where Active=1 AND ID=" + dr["ItemID"] + "";
                ...
                {
                    DataTable dt = new DataTable();
                    string sQry = "Select ID,(Select Name from ItemGroup where ID=ItemGroupID)as [Group],Catalog,Name, IsNull(FeePrice,0) as Price from Fees WHERE ID= " + dr["FeeID"] + " and Active=1";
                    SqlDataAdapter sqDA = new SqlDataAdapter(sQry, this.connectionString);
                    sqDA.Fill(dt);
                    dr["Catalog"] = dt.Rows[0]["Catalog"];
                    dr["Description"] = dt.Rows[0]["Name"];
                ...
                {
                    DataTable dt = new DataTable();
                    string sQry = "Select ID,(Select Name from ItemGroup where ID=ItemGroupID)as [Group],Catalog,Name,LaborHours, IsNull(LaborFees,0) as Fee from Labor WHERE ID=" + dr["laborID"] + " and Active=1";
                    SqlDataAdapter sqDA = new SqlDataAdapter(sQry, this.connectionString);
                    sqDA.Fill(dt);
                    dr["Catalog"] = dt.Rows[0]["Catalog"];
                    dr["Description"] = dt.Rows[0]["Name"];
        ...
        public DataTable GetJournalTransactions(int InvoiceID, int AccountID)
        {
            DataTable dataTable = new DataTable();
            string Qry = "Select SubAccount as [G/L Account],Debit,Credit from AccountJournalTransactions acc inner join AccountVoucher av on acc.InvoiceID=av.InvoiceID where acc.InvoiceID=" + InvoiceID + " AND AV.AccountID=" + AccountID;

            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            return dataTable;
        }
        public bool UpdateLedger(DataTable vdt, DataTable vddt, DataTable Accountdt, decimal Tamount, decimal Tparts, decimal Twheels, decimal TTire, decimal TLabour, decimal TFET, decimal TFEE, decimal TOparts, decimal TOthers, decimal Ttax)
        {
            decimal TotalCash = 0;
            if (vdt.Rows.Count > 0)
            {
                if (vdt.Rows[0]["Receivable"] != System.DBNull.Value)
                {
                    TotalCash = Tamount - Convert.ToDecimal(vdt.Rows[0]["Receivable"]);
                }
                else
                { TotalCash = Tamount; }
            }
            else { TotalCash = Tamount; }
            SqlConnection SqlCnn = new SqlConnection(this.connectionString);
            ...
            try
            {
                ...
                int wID = 0;
                DataTable dt = new DataTable();
                string CQry = "Select * from AccountVoucher where InvoiceID = " + vdt.Rows[0]["InvoiceID"] + "";
                SqlDataAdapter da = new SqlDataAdapter(CQry,this.connectionString);
                da.Fill(dt);
                if (dt.Rows.Count == 0)
                {
                    SqlCommand vcommand = new SqlCommand("INSERT INTO [dbo].[AccountVoucher]([vNo],[vDate],[vTypeID],[InvoiceID],[WOID],[PID],[AccountID],[BankAccountID]," +
                        "[vforVendor],[vforCustomer],[vforEmployee],[CustomerID],[VendorID],[EmployeeID],[Narration],[TPAmount],[TLAmount],[TFET]," +
                        "[TTaxable],[TTax],[TDiscount],[TotalAmount],[TotalProfit],[PaidAmount],[BillBalance],[AmountIn],[AmountOut],[PayByCash],[PaybyBank],[Receivable],[TrnsNotes],[Active],[AddDate],[AddUserID],[ModifyUserID],[ModifyDate],[Comments],[IsLocked]," +
                        "[CompanyID],[WarehouseID],[StoreID])" +

                        " VALUES (@vNo ,@vDate ,@vTypeID , @InvoiceID ,@WOID,@PID,@AccountID ,@BankAccountID ,@vforVendor ,@vforCustomer ," +
                        " @vforEmployee ,@CustomerID,@VendorID ,@EmployeeID ,@Narration ,@TPAmount,@TLAmount,@TFET,@TTaxable,@TTax,@TDiscount,@TotalAmount,@TotalProfit,@PaidAmount,@BillBalance,@AmountIn, @AmountOut,@PayByCash,@PaybyBank,@Receivable,@TrnsNotes," +
                        " @Active ,@AddDate,@AddUserID ," +
                        " @ModifyUserID,@ModifyDate,@Comments ,@IsLocked," + StaticInfo.CompanyID + " ," + StaticInfo.WarehouseID + "  ," +
                    ...
                    vcommand.Parameters.AddWithValue("@vNo", vdt.Rows[0]["vNo"]);
                    vcommand.Parameters.AddWithValue("@vDate", vdt.Rows[0]["vDate"]);
                    vcommand.Parameters.AddWithValue("@vTypeID", vdt.Rows[0]["vTypeID"]);
                    vcommand.Parameters.AddWithValue("@InvoiceID", vdt.Rows[0]["InvoiceID"]);
                    vcommand.Parameters.AddWithValue("@WOID", vdt.Rows[0]["WOID"]);
                    vcommand.Parameters.AddWithValue("@PID", vdt.Rows[0]["PID"]);
                    vcommand.Parameters.AddWithValue("@AccountID", vdt.Rows[0]["AccountID"]);
                    vcommand.Parameters.AddWithValue("@BankAccountID", vdt.Rows[0]["BankAccountID"]);

                    vcommand.Parameters.AddWithValue("@vforVendor", vdt.Rows[0]["vforVendor"]);
                    vcommand.Parameters.AddWithValue("@vforCustomer", vdt.Rows[0]["vforCustomer"]);
                    vcommand.Parameters.AddWithValue("@vforEmployee", vdt.Rows[0]["vforEmployee"]);

                    vcommand.Parameters.AddWithValue("@CustomerID", vdt.Rows[0]["CustomerID"]);
                    vcommand.Parameters.AddWithValue("@VendorID", vdt.Rows[0]["VendorID"]);
                    vcommand.Parameters.AddWithValue("@EmployeeID", vdt.Rows[0]["EmployeeID"]);
                    vcommand.Parameters.AddWithValue("@Narration", vdt.Rows[0]["Narration"]);

                    vcommand.Parameters.AddWithValue("@TPAmount", vdt.Rows[0]["TPAmount"]);
                    vcommand.Parameters.AddWithValue("@TLAmount", vdt.Rows[0]["TLAmount"]);
                    vcommand.Parameters.AddWithValue("@TFET", vdt.Rows[0]["TFET"]);
                    vcommand.Parameters.AddWithValue("@TTaxable", vdt.Rows[0]["TTaxable"]);
                    vcommand.Parameters.AddWithValue("@TTax", vdt.Rows[0]["TTax"]);
                    vcommand.Parameters.AddWithValue("@TDiscount", vdt.Rows[0]["TDiscount"]);
                    vcommand.Parameters.AddWithValue("@TotalAmount", vdt.Rows[0]["TotalAmount"]);
                    vcommand.Parameters.AddWithValue("@TotalProfit", vdt.Rows[0]["TotalProfit"]);

                    vcommand.Parameters.AddWithValue("@PaidAmount", vdt.Rows[0]["PaidAmount"]);
                    vcommand.Parameters.AddWithValue("@BillBalance", vdt.Rows[0]["BillBalance"]);

                    vcommand.Parameters.AddWithValue("@AmountIn", vdt.Rows[0]["AmountIn"]);
                    vcommand.Parameters.AddWithValue("@AmountOut", vdt.Rows[0]["AmountOut"]);
                    vcommand.Parameters.AddWithValue("@PayByCash", vdt.Rows[0]["PayByCash"]);
                    vcommand.Parameters.AddWithValue("@PaybyBank", vdt.Rows[0]["PaybyBank"]);
                    vcommand.Parameters.AddWithValue("@Receivable", vdt.Rows[0]["Receivable"]);
                    vcommand.Parameters.AddWithValue("@TrnsNotes", vdt.Rows[0]["TrnsNotes"]);

                    vcommand.Parameters.AddWithValue("@Active", vdt.Rows[0]["Active"]);
                    vcommand.Parameters.AddWithValue("@AddDate", vdt.Rows[0]["AddDate"]);
                    vcommand.Parameters.AddWithValue("@AddUserID", vdt.Rows[0]["AddUserID"]);
                    vcommand.Parameters.AddWithValue("@ModifyUserID", vdt.Rows[0]["ModifyUserID"]);
                    vcommand.Parameters.AddWithValue("@ModifyDate", vdt.Rows[0]["ModifyDate"]);
                    vcommand.Parameters.AddWithValue("@Comments", vdt.Rows[0]["Comments"]);
                    vcommand.Parameters.AddWithValue("@IsLocked", vdt.Rows[0]["IsLocked"]);

                    vcommand.Transaction = trans;
                    vstatus = vcommand.ExecuteNonQuery();
                    ...
                    if (vstatus != 0)
                    {
                        ...
                        for (int j = 0; j < vddt.Rows.Count; j++)
                        {
                            SqlCommand vdcommand = new SqlCommand("INSERT INTO [dbo].[AccountVoucherDetails] ([MID],[vDate],[vTypeID],[InvoiceID],[WOID],[PID],[ItemID],[vforVendor],[vforCustomer],[vforEmployee],[Ctype],[Available],[Qty],[Price],[Cost],[Amount],[DiscAmount],[FET],[Total],[Profit],[Tax]" +
                                ",[AmountIn],[AmountOut],[TrnsNotes],[Active],[AddDate],[AddUserID]" +
                                ",[ModifyUserID],[ModifyDate],[Comments],[IsLocked],[DocNo],[Remarks],[CompanyID],[WarehouseID],[StoreID])" +

                                "VALUES(@MID,@vDate,@vTypeID,@InvoiceID ,@WOID,@PID,@ItemID,@vforVendor,@vforCustomer,@vforEmployee,@Ctype,@Available,@Qty,@Price,@Cost,@Amount,@DiscAmount,@FET,@Total,@Profit,@Tax," +
                                "@AmountIn,@AmountOut," +
                                "@TrnsNotes ,@Active ,@AddDate,@AddUserID ," +
                                "@ModifyUserID,@ModifyDate,@Comments ,@IsLocked,@DocNo,@Remarks," + StaticInfo.CompanyID + " ," + StaticInfo.WarehouseID + "  ," +
                                "" + StaticInfo.StoreID + ")", SqlCnn);
                            if (j == 0)
                            {
                                Vocherno = Convert.ToInt32(vddt.Rows[j]["MID"]);
                                if (vddt.Rows[j]["WOID"] != System.DBNull.Value)
                                {
                                    wID = Convert.ToInt32(vddt.Rows[j]["WOID"]);
                                }
                            }
                            //,GETDATE() ," + StaticInfo.userid + ",@Comments ,0 ," + StaticInfo.CompanyID + " ," + StaticInfo.WarehouseID + " ," + StaticInfo.StoreID + ")
                            vdcommand.Parameters.AddWithValue("@MID", vddt.Rows[j]["MID"]);
                            vdcommand.Parameters.AddWithValue("@vDate", vddt.Rows[j]["vDate"]);
                            vdcommand.Parameters.AddWithValue("@vTypeID", vddt.Rows[j]["vTypeID"]);
                            vdcommand.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                            vdcommand.Parameters.AddWithValue("@WOID", vddt.Rows[j]["WOID"]);
                            vdcommand.Parameters.AddWithValue("@PID", vddt.Rows[j]["PID"]);

                            vdcommand.Parameters.AddWithValue("@ItemID", vddt.Rows[j]["ItemID"]);
                            vdcommand.Parameters.AddWithValue("@vforVendor", vddt.Rows[j]["vforVendor"]);
                            vdcommand.Parameters.AddWithValue("@vforCustomer", vddt.Rows[j]["vforCustomer"]);
                            vdcommand.Parameters.AddWithValue("@vforEmployee", vddt.Rows[j]["vforEmployee"]);

                            vdcommand.Parameters.AddWithValue("@Ctype", vddt.Rows[j]["Ctype"]);
                            vdcommand.Parameters.AddWithValue("@Available", vddt.Rows[j]["Available"]);
                            vdcommand.Parameters.AddWithValue("@Qty", vddt.Rows[j]["Qty"]);

                            vdcommand.Parameters.AddWithValue("@Price", vddt.Rows[j]["Price"]);
                            vdcommand.Parameters.AddWithValue("@Cost", vddt.Rows[j]["Cost"]);
                            vdcommand.Parameters.AddWithValue("@Amount", vddt.Rows[j]["Amount"]);
                            vdcommand.Parameters.AddWithValue("@DiscAmount", vddt.Rows[j]["DiscAmount"]);
                            vdcommand.Parameters.AddWithValue("@FET", vddt.Rows[j]["FET"]);

                            vdcommand.Parameters.AddWithValue("@Total", vddt.Rows[j]["Total"]);
                            vdcommand.Parameters.AddWithValue("@Profit", vddt.Rows[j]["Profit"]);
                            vdcommand.Parameters.AddWithValue("@Tax", vddt.Rows[j]["Tax"]);

                            vdcommand.Parameters.AddWithValue("@AmountIn", vddt.Rows[j]["AmountIn"]);
                            vdcommand.Parameters.AddWithValue("@AmountOut", vddt.Rows[j]["AmountOut"]);

                            vdcommand.Parameters.AddWithValue("@TrnsNotes", vddt.Rows[j]["TrnsNotes"]);
                            vdcommand.Parameters.AddWithValue("@Active", vddt.Rows[j]["Active"]);
                            vdcommand.Parameters.AddWithValue("@AddDate", vddt.Rows[j]["AddDate"]);
                            vdcommand.Parameters.AddWithValue("@AddUserID", vddt.Rows[j]["AddUserID"]);

                            vdcommand.Parameters.AddWithValue("@ModifyUserID", vddt.Rows[j]["ModifyUserID"]);
                            vdcommand.Parameters.AddWithValue("@ModifyDate", vddt.Rows[j]["ModifyDate"]);
                            vdcommand.Parameters.AddWithValue("@Comments", vddt.Rows[j]["Comments"]);
                            vdcommand.Parameters.AddWithValue("@IsLocked", vddt.Rows[j]["IsLocked"]);
                            vdcommand.Parameters.AddWithValue("@DocNo", vddt.Rows[j]["DocNo"]);
                            vdcommand.Parameters.AddWithValue("@Remarks", vddt.Rows[j]["Remarks"]);

                            vdcommand.Transaction = trans;
                            vdstatus = vdcommand.ExecuteNonQuery();

                            if (vdstatus != 0)
                            {
                                if (vdt.Rows[0]["PID"] == System.DBNull.Value && vdt.Rows[0]["WOID"] != System.DBNull.Value)
                                {
                                    if (SqlCnn.State == ConnectionState.Closed)
                                    {
                                        SqlCnn.Open();
                                    }
                                    if (j == 0 && !string.IsNullOrEmpty(Convert.ToString(Tamount)) && Convert.ToString(Tamount) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        comm.Parameters.AddWithValue("@AccountID", Accountdt.Rows[0]["ID"]);
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[0]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", TotalCash);
                                        comm.Parameters.AddWithValue("@Credit", "0.00");
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (j == 0 && !string.IsNullOrEmpty(Convert.ToString(TFET)) && Convert.ToString(TFET) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        comm.Parameters.AddWithValue("@AccountID", Accountdt.Rows[1]["ID"]);
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[2]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", TFET);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (j == 0 && !string.IsNullOrEmpty(Convert.ToString(Ttax)) && Convert.ToString(Ttax) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        ...
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[9]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", Ttax);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (j == 0 && !string.IsNullOrEmpty(Convert.ToString(vdt.Rows[0]["Receivable"])) && Convert.ToString(vdt.Rows[0]["Receivable"]) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        ...
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[10]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", vdt.Rows[0]["Receivable"]);
                                        comm.Parameters.AddWithValue("@Credit", "0.00");
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (vddt.Rows[j]["Ctype"].ToString() == "Par" && !string.IsNullOrEmpty(Convert.ToString(Tparts)) && Convert.ToString(Tparts) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        ...
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[4]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", Tparts);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (vddt.Rows[j]["Ctype"].ToString() == "Whe" && !string.IsNullOrEmpty(Convert.ToString(Twheels)) && Convert.ToString(Twheels) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        ...
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[5]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", Twheels);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (vddt.Rows[j]["Ctype"].ToString() == "Tir" && !string.IsNullOrEmpty(Convert.ToString(TTire)) && Convert.ToString(TTire) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        ...
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[6]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", TTire);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (vddt.Rows[j]["Ctype"].ToString() == "Lub" && !string.IsNullOrEmpty(Convert.ToString(Tparts)) && Convert.ToString(Tparts) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                    ...
                                    if (vddt.Rows[j]["Ctype"].ToString() == "Fees" && !string.IsNullOrEmpty(Convert.ToString(TFEE)) && Convert.ToString(TFEE) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        ...
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[9]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", TFEE);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (vddt.Rows[j]["Ctype"].ToString() == "Lab" && !string.IsNullOrEmpty(Convert.ToString(TLabour)) && Convert.ToString(TLabour) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        ...
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[3]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", TLabour);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (vddt.Rows[j]["Ctype"].ToString() == "Out" && !string.IsNullOrEmpty(Convert.ToString(TOparts)) && Convert.ToString(TOparts) != "0")
                                    {
                                        SqlCommand comm = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,AccountID,InvoiceID,vDate,SubAccount,Debit,Credit,Remarks) " +
                                            "VALUES(@vNo,@AccountID,@InvoiceID,@vDate,@SubAccount,@Debit,@Credit,@Remarks)", SqlCnn);
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        ...
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["InvoiceID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[7]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", TOthers);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                ...
                                else
                                {
                                    ...
                                    if (j == 0 && !string.IsNullOrEmpty(Convert.ToString(Tamount)) && Convert.ToString(Tamount) != "0")
                                    {
                                        ...
                                        comm.Parameters.AddWithValue("@vNo", Vocherno);
                                        comm.Parameters.AddWithValue("@AccountID", Accountdt.Rows[0]["ID"]);
                                        comm.Parameters.AddWithValue("@InvoiceID", vddt.Rows[j]["PID"]);
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[0]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", "0.00");
                                        comm.Parameters.AddWithValue("@Credit", vdt.Rows[0]["TotalAmount"]);
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
                                    }
                                    if (vddt.Rows[j]["Ctype"].ToString() == "Par" && !string.IsNullOrEmpty(Convert.ToString(Tparts)) && Convert.ToString(Tparts) != "0")
                                    {
                                        ...
                                        comm.Parameters.AddWithValue("@vDate", DateTime.Now);
                                        comm.Parameters.AddWithValue("@SubAccount", Accountdt.Rows[4]["AccName"]);
                                        comm.Parameters.AddWithValue("@Debit", vddt.Rows[j]["Amount"]);
                                        comm.Parameters.AddWithValue("@Credit", "0.00");
                                        comm.Parameters.AddWithValue("@Remarks", "");
                                        comm.Transaction = trans;
                                        int jstatus = comm.ExecuteNonQuery();
        ...
        public DataTable GetBillsDetailsForLiability(int BillID)
        {
            DataTable dt = new DataTable();
            try
            {
                string qry = @"SELECT VBD.ID, VBD.MID,0 As PaymentID, VBD.ItemID, I.Catalog,IT.Initial,VBD.BillQty, VBD.CatalogCost, VBD.BillAmount, VB.Discount,
                            VB.BillTotalAmount, VBD.AddDate, VBD.AddUserID, VBD.Comments
                            From VendorBillDetails VBD INNER join VendorBill VB on VBD.MID = VB.ID  left join Item I on I.ID = VBD.ItemID
                            left join ItemType IT on I.ItemTypeID = IT.ID
                            WHERE VBD.MID = "+ BillID + " AND VBD.Active = 1";
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    sqlCon.Open();
                    SqlCommand sqlCmd = new SqlCommand(qry, sqlCon);
        ...
        public DataTable GetBillsForLiability(String Date)
        {
            DataTable dt = new DataTable();
            try
            {
                string qry = @"SELECT BillID,BillDate,(Select AccName from Account where ID = 2) AS Account,BillTotalAmount,
                            Discount,PaidAmount,Balance from VendorBill
							where Balance !='0.00' AND BillDate='"+ Date + "'";
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    sqlCon.Open();
                    SqlCommand sqlCmd = new SqlCommand(qry, sqlCon);
        ...
        public DataTable GetVendorPaymentForVoucherDetails(int PaymentID)
        {
            DataTable dt = new DataTable();
            try
            {
                string qry = @"SELECT VBD.ID,VP.PaymentID, VBD.MID, VBD.ItemID,VP.InvoiceNo, I.Catalog,IT.Initial,VBD.BillQty, VBD.CatalogCost, VBD.BillAmount, VB.Discount,
                            VB.BillTotalAmount, VBD.AddDate, VBD.AddUserID, VBD.Comments
                            From VendorPayment VP left join VendorBill VB on VP.BillID = VB.BillID inner join
                            VendorBillDetails VBD on VB.ID = VBD.MID left join Item I on I.ID = VBD.ItemID
                            left join ItemType IT on I.ItemTypeID = IT.ID
                            WHERE VP.paymentID = " + PaymentID + " AND VP.Active = 1";
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    sqlCon.Open();
                    SqlCommand sqlCmd = new SqlCommand(qry, sqlCon);
        ...
        public DataTable GetVendorPaymentHistorForVoucher(int PaymentID)
        {
            DataTable dt = new DataTable();
            try
            {
                string qry = @"SELECT ID,InvoiceNo,PaymentID,TrnsDate,(Select AccName from Account where ID = 4) AS Account,
                            (Select top(1)ID from AccountVoucherTypes) as vTypeID,BillAmount,
                            BillDiscount,PaidAmount,BillBalance,PayByCash,PaybyCheck,PayByVisa, PayByAMEX,PayByATM,PayByDSCVR,PayByMC,
                            TrnsNotes,IsLocked from VendorPaymentHistory
                            WHERE PaymentID = "+ PaymentID + "";
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    sqlCon.Open();
        ...
        public bool AddVoucherDetails(DataTable dt, decimal Tamount, decimal Tparts, decimal TOparts, decimal Twheels, decimal Ttire, decimal TFET, decimal TFEE, decimal TLabour, decimal TOthers, decimal Ttax)
        {
            ...
            try
            {
                ...
                {
                    SqlCommand command = new SqlCommand("INSERT INTO [dbo].[AccountVoucherDetails] ([vNo],[MID],[vtype],[vDate],[ItemID],[FeeID],[LabourID],[Qty],[Hours],[Price],[Cost],[Amount],[Discount],[FET],[Total],[Tax],[SubAccount]" +
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        try
                        {
                            if (i == 0 && !string.IsNullOrEmpty(Convert.ToString(Tamount)) && Convert.ToString(Tamount) != "0")
                            {
                                ...
                                SqlCommand command = new SqlCommand("INSERT INTO [dbo].[AccountJournalTransactions] (vNo,wID,vtype,vDate,SubAccount,Debit,Credit,Remarks) " +
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = "Select Top(1) wo.Tax,wo.IsLocked from workorder wo right outer join WorkOrderDetail wod on wo.ID=wod.MID where wod.MID='" + ID + "' AND wo.CompanyID=" + StaticInfo.CompanyID + " AND CAST(wod.AddDate as DATE)='" + date + "'";
        ...
        public DataTable GetSaleDetailsByInvoiceID(int InvoiceID)
        {
            DataTable dt = new DataTable();
            string Qry= "Select CR.InvoiceNo,cr.WOID,cr.TrnsDate,cr.TrnsNotes,cr.ChgOnAccount,cr.TotalReceivedAmount,cr.CusCredit,cr.IsCredit,wo.PartsPrice,wo.LaborPrice,wo.FET,wo.Tax,WO.Total,WO.TotalProfit,WO.Status,WO.Active,WO.AddUserID,WO.IsLocked,WO.IsCustomerOrder,WO.CustomerID,WO.CompanyID,WO.StoreID,WO.WarehouseID from CustomerReceipt cr inner join WorkOrder wo on cr.WOID=wo.ID WHERE CR.InvoiceNo='"+ InvoiceID +"'";
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dt);
            return dt;
        }
        ...
        public DataTable getItemStockStatusByWOID(int WOID)
        {
            DataTable dataTable = new DataTable();
            string Qry = " Select S.QtyOnHand From WorkOrder WO Left join WorkOrderDetail WOD On WO.ID = WOD.MID" +
                         " Left join ItemStock S On WOD.ItemID = S.ItemID Where WO.ID = " + WOID;
        ...
        public DataTable getNegatedInvoicesBySearch(string searchKey)
        {
            DataTable dataTable = new DataTable();
            string Qry = " SELECT ID ,[TransNo] ,[Date] ,[InvoiceNo] ,[Type] ,CAST('False' as bit) [IsCredit] ,[SaleRepID] ,[Paid by], [ComputerIP],[CustomerID],(Select (FirstName+LastName) from Customer where ID=CustomerID) as CustomerName  ,[Rep] ,[PONo] ,[Mileage] ,[VehicleID] ,[Vehicle] ,[Open Amt] ,[Total Amt] ,[MechID] ,[Mech]" +
                         ...
                         " ,[Type] = CASE WO.[IsQutation] WHEN 1 THEN 'Qutation' ELSE CASE WO.[IsWorkOrderNegate] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerPayment" +
                         " where WONID = WO.ID) THEN 'WorkOrderNegate' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerPayment where WONID = WO.ID)" +
                         " THEN 'Negated Invoice' END ELSE CASE WO.[IsCustomerOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'CusOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'Negated Invoice' END  ELSE '' END END END ,[WO].[WorkOrderNegateNo] ,WO.[SaleRepID]" +
                         " ,cus.CompanyName [Paid by] , WO.[ComputerIP],WO.[CustomerID] ,emp.Initial [Rep]" +
                         " ,[PONo] = 'Negate #'+(select Convert(varchar(50), WorkOrderNo) from dbo.WorkOrder where ID = wo.WorkOrderID)" +
                         " ,WO.[Mileage] ,WO.[VehicleID]" +
                         " ,[Vehicle] = vy.Name + ' ' + vmk.Name + ' ' +vmd.Name" +
                         ...
                         " ,(WO.[Total]*-1) [Total Amt]" +
                         " ,WO.[MechID] ,emp1.Initial [Mech] ,WO.[Status] ,WO.Notes [Description] ,WO.[SaleCategoryID] ,sCat.Name [Category] ,WO.[ShipViaID] ,sVia.Name [ShipVia] ,cus.Phone1 [Phone] ,WO.[StoreID] ,wStr.CoName [Store],[IsNegated] = 0, WO.[IsLocked]" +
                         " FROM [dbo].[CustomerPayment] [CP] Left Join [dbo].[WorkOrderNegate] [WO] On CP.WONID = WO.ID Left Join dbo.Customer cus on wo.CustomerID = cus.ID Left Join dbo.Employee emp on WO.[SaleRepID] = emp.ID Left Join dbo.Vehicle veh on wo.VehicleID = veh.ID Left Join dbo.VehicleYear vy on veh.VehicleYearID = vy.ID Left Join dbo.VehicleMake vmk on veh.VehicleMakeID = vmk.ID Left Join dbo.VehicleModel vmd on veh.VehicleModelID = vmd.ID Left Join dbo.Employee emp1 on WO.[MechID] = emp1.ID Left Join dbo.SaleTaxCategory sCat on WO.[SaleCategoryID] = sCat.ID Left Join dbo.ShipVia sVia on WO.[ShipViaID] = sVia.ID Left Join dbo.WarehouseStore wStr on WO.StoreID = wStr.ID" +
                         " WHERE CP.InvoiceNo like '%" + searchKey + "%') tbl1";
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            try
            {
        ...
        public DataTable getInvoicesBySearch(string searchKey)
        {
            DataTable dataTable = new DataTable();
            string Qry = " SELECT ID ,[TransNo] ,[Date] ,[InvoiceNo] ,[Type] ,[IsCredit] ,[SaleRepID] ,[Paid by] ,[CustomerID] ,[Rep] ,[PONo] ,[Mileage] ,[VehicleID] ,[Vehicle] ,[Open Amt] ,[Total Amt] ,[MechID] ,[Mech]" +
                         " ,[Status] = CASE [Type] WHEN 'Invoice' THEN 'Completed'  ELSE [Status] END ,[Description] ,[SaleCategoryID] ,[Category] ,[ShipViaID] ,[ShipVia] ,[Phone] ,[StoreID] ,[Store]" +
                         " ,[ctrName] = CASE [Type] WHEN 'Invoice' THEN 'ctrCustomerReceipt' ELSE 'ctrWorkOrder' END" +
                         " ,[IsNegated], [IsLocked]" +
                         " FROM" +
                         " (SELECT WO.[ID]" +
                         " ,[WO].[WorkOrderNo] [TransNo] ,WO.[RegDate] [Date], CR.[InvoiceNo] ,Isnull(CR.[IsCredit],0) as IsCredit" +
                         " ,[Type] = CASE WO.[IsQutation] WHEN 1 THEN 'Qutation' ELSE CASE WO.[IsWorkOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt" +
                         " where WOID = WO.ID) THEN 'WorkOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID)" +
                         " THEN 'Invoice' END ELSE CASE WO.[IsCustomerOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'CusOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'Invoice' END  ELSE '' END END END ,[WO].[WorkOrderNo] ,WO.[SaleRepID]" +
                         " ,cus.CompanyName [Paid by] ,WO.[CustomerID] ,emp.Initial [Rep] ,WO.[PONo] ,WO.[Mileage] ,WO.[VehicleID]" +
                         ...
                         " ,WO.[Total] [Total Amt]" +
                         " ,WO.[MechID] ,emp1.Initial [Mech] ,WO.[Status] ,WO.Notes [Description] ,WO.[SaleCategoryID] ,sCat.Name [Category] ,WO.[ShipViaID] ,sVia.Name [ShipVia] ,cus.Phone1 [Phone] ,WO.[StoreID] ,wStr.CoName [Store],WO.[IsNegated], WO.[IsLocked]" +
                         " FROM [dbo].[CustomerReceipt] [CR] Left join  [dbo].[WorkOrder] [WO] On CR.WOID = WO.ID  Left Join dbo.Customer cus on wo.CustomerID = cus.ID Left Join dbo.Employee emp on WO.[SaleRepID] = emp.ID Left Join dbo.Vehicle veh on wo.VehicleID = veh.ID Left Join dbo.VehicleYear vy on veh.VehicleYearID = vy.ID Left Join dbo.VehicleMake vmk on veh.VehicleMakeID = vmk.ID Left Join dbo.VehicleModel vmd on veh.VehicleModelID = vmd.ID Left Join dbo.Employee emp1 on WO.[MechID] = emp1.ID Left Join dbo.SaleTaxCategory sCat on WO.[SaleCategoryID] = sCat.ID Left Join dbo.ShipVia sVia on WO.[ShipViaID] = sVia.ID Left Join dbo.WarehouseStore wStr on WO.StoreID = wStr.ID" +
                         " WHERE CR.InvoiceNo like '%" + searchKey + "%') tbl";
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            try
            {
        ...
        public DataTable getWorkOrderBySearch(string searchKey)
        {
            DataTable dataTable = new DataTable();
            string Qry = " SELECT ID ,[TransNo] ,[Date] ,[InvoiceNo] ,[Type] ,CAST('False' as bit) [IsCredit] ,[SaleRepID] ,[Paid by] ,[CustomerID] ,[Rep] ,[PONo] ,[Mileage] ,[VehicleID] ,[Vehicle] ,[Open Amt] ,[Total Amt] ,[MechID] ,[Mech]" +
                         " ,[Status] = CASE [Type] WHEN 'Invoice' THEN 'Completed'  ELSE [Status] END ,[Description] ,[SaleCategoryID] ,[Category] ,[ShipViaID] ,[ShipVia] ,[Phone] ,[StoreID] ,[Store]" +
                         " ,[ctrName] = CASE [Type] WHEN 'Invoice' THEN 'ctrCustomerReceipt' ELSE 'ctrWorkOrder' END" +
                         " ,[IsNegated], [IsLocked]" +
                         " FROM" +
                         " (SELECT WO.[ID]" +
                         " ,[WO].[WorkOrderNo] [TransNo] ,WO.[RegDate] [Date], ''[InvoiceNo]" +
                         " ,[Type] = CASE WO.[IsQutation] WHEN 1 THEN 'Qutation' ELSE CASE WO.[IsWorkOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt" +
                         " where WOID = WO.ID) THEN 'WorkOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID)" +
                         " THEN 'Invoice' END ELSE CASE WO.[IsCustomerOrder] WHEN 1 THEN CASE WHEN WO.[Total] > (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'CusOrder' WHEN WO.[Total] <= (select ISNULL(SUM(TotalReceivedAmount),0) from dbo.CustomerReceipt where WOID = WO.ID) THEN 'Invoice' END  ELSE '' END END END ,[WO].[WorkOrderNo] ,WO.[SaleRepID]" +
                         " ,cus.CompanyName [Paid by] ,WO.[CustomerID] ,emp.Initial [Rep] ,WO.[PONo] ,WO.[Mileage] ,WO.[VehicleID]" +
                         ...
                         " ,WO.[MechID] ,emp1.Initial [Mech] ,WO.[Status] ,WO.Notes [Description] ,WO.[SaleCategoryID] ,sCat.Name [Category] ,WO.[ShipViaID] ,sVia.Name [ShipVia] ,cus.Phone1 [Phone] ,WO.[StoreID] ,wStr.CoName [Store],WO.[IsNegated], WO.[IsLocked]" +
                         " FROM [dbo].[WorkOrder] [WO] Left Join dbo.Customer cus on wo.CustomerID = cus.ID Left Join dbo.Employee emp on WO.[SaleRepID] = emp.ID Left Join dbo.Vehicle veh on wo.VehicleID = veh.ID Left Join dbo.VehicleYear vy on veh.VehicleYearID = vy.ID Left Join dbo.VehicleMake vmk on veh.VehicleMakeID = vmk.ID Left Join dbo.VehicleModel vmd on veh.VehicleModelID = vmd.ID Left Join dbo.Employee emp1 on WO.[MechID] = emp1.ID Left Join dbo.SaleTaxCategory sCat on WO.[SaleCategoryID] = sCat.ID Left Join dbo.ShipVia sVia on WO.[ShipViaID] = sVia.ID Left Join dbo.WarehouseStore wStr on WO.StoreID = wStr.ID" +
                         " WHERE WO.WorkOrderNo like '%" + searchKey + "%') tbl";
        ...
        public bool UpdateWorkOrderDetailsPosted(string SelectedDate)
        {
            ...
            {
                SqlCnn.Open();
                string strQry = "UPDATE [dbo].[CustomerReceipt] SET [IsLocked] = 1 WHERE CAST(TrnsDate AS DATE) = '" + SelectedDate + "'";
        ...
        public void UpdateCustomerRecieptCredit(int WONID)
        {
            SqlConnection SqlCnn = new SqlConnection(this.connectionString);
            try
            {
                ...
                SqlCommand Cmd = new SqlCommand("Update CustomerReceipt Set IsCredit=0 Where ID = (Select CR.ID from CustomerReceipt CR Right Join WorkOrder WO on CR.WOID=WO.ID Right Join WorkOrderNegate WON on WO.ID= WON.WorkOrderID  Where WON.ID=@ID and CR.CompanyID=@Company)", SqlCnn);

                Cmd.Parameters.Add("@ID", SqlDbType.Int);
                Cmd.Parameters["@ID"].Value = WONID;

                Cmd.Parameters.Add("@Company", SqlDbType.Int);
                Cmd.Parameters["@Company"].Value = StaticInfo.CompanyID;
        ...
        public decimal getCreditAvailbyCustomerID(int CustID)
        {
            ...
            try
            {
                string qry = "Select(SELECT ISNULL(SUM(ChgOnAccount), 0)" +
                             " FROM[dbo].[CustomerReceipt]" +
                             " WHERE IsCredit=1 and CustomerID = " + CustID + ") - (SELECT ISNULL(SUM(ChgOnAccount), 0)" +
                             " FROM[dbo].[CustomerPayment]" +
                             " WHERE CustomerID = " + CustID + ") - (SELECT ISNULL(SUM(PaidAmount), 0)" +
        ...
        public decimal getPastDueAmountbyCustomerID(int CustID)
        {
            DateTime datefrom = DateTime.Now;
            decimal Amount = 0;
            try
            {
                string qry = "Select (Select isnull(sum(ChgOnAccount),0) from CustomerReceipt left join Customer " +
                             " On Customer.ID=CustomerReceipt.CustomerID left join Terms on Customer.SaleTermID=Terms.ID " +
                             " Where CustomerReceipt.TrnsDate >= " +
                             " (select(cast(DATEADD(day, -1 * Isnull(Terms.DueByDaysD1, 0), " +
                             "'" + Convert.ToString(datefrom.Date.Year + "-" + datefrom.Date.Month + "-" + datefrom.Date.Day) + "'" +
                             " ) as date)))" +
                             " and CustomerReceipt.TrnsDate <= " +
                             "'" + Convert.ToString(datefrom.Date.Year + "-" + datefrom.Date.Month + "-" + datefrom.Date.Day) + "'" +
                             " And CustomerReceipt.CustomerID = " + CustID + ")" +
                             " - " +
                             " (Select isnull(sum(ChgOnAccount), 0) from CustomerPayment left join Customer" +
                             "  On Customer.ID = CustomerPayment.CustomerID left join Terms on Customer.SaleTermID = Terms.ID" +
                             "   Where CustomerPayment.TrnsDate >= " +
                             " (select(cast(DATEADD(day, -1 * Isnull(Terms.DueByDaysD1, 0), " +
                             "'" + Convert.ToString(datefrom.Date.Year + "-" + datefrom.Date.Month + "-" + datefrom.Date.Day) + "'" +
                             " ) as date)))" +
                             " and CustomerPayment.TrnsDate <= " +
                             "'" + Convert.ToString(datefrom.Date.Year + "-" + datefrom.Date.Month + "-" + datefrom.Date.Day) + "'" +
                             " And CustomerPayment.CustomerID = " + CustID + ")" +
                             " - " +
                             " (Select isnull(sum(PaidAmount), 0) from CustomerCreditHistory left join Customer" +
                             " On Customer.ID = CustomerCreditHistory.CustomerID left join Terms on Customer.SaleTermID = Terms.ID" +
                             "    Where CustomerCreditHistory.TrnsDate >= " +
                             "   (select(cast(DATEADD(day, -1 * Isnull(Terms.DueByDaysD1, 0), " +
                             "'" + Convert.ToString(datefrom.Date.Year + "-" + datefrom.Date.Month + "-" + datefrom.Date.Day) + "'" +
                             "   ) as date)))" +
                             " and CustomerCreditHistory.TrnsDate <= " +
                             "'" + Convert.ToString(datefrom.Date.Year + "-" + datefrom.Date.Month + "-" + datefrom.Date.Day) + "'" +
                             " And CustomerCreditHistory.CustomerID = " + CustID + ")";
                ...
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    ...
                    sqlCon.Close();
                    //if (Convert.ToDecimal(xResult) > 0)
                    Amount = Convert.ToDecimal(xResult);
                }
            }
            catch (Exception ex) { throw ex; }
            return Amount;
        ...
        public int getNextCustomerReceiptAutoNo()
        {
            ...
            try
            {
                string qry = "SELECT IsNull(MAX(ReceiptID),0)+1 FROM  CustomerReceipt";
        ...
        {
            int NextAutoNo = 0;
            String Qry = " Select IsNull(max(WorkOrderNo),0) + 1 as MaxWorkOrderNo from WorkOrder";
            using (SqlConnection conn = new SqlConnection(this.connectionString))
            {
                conn.Open();
        ...
        public int getNextWorkOrderNegateAutoNo()
        {
            ...
            String Qry = " Select IsNull(max(WorkOrderNegateNo),0) + 1 as MaxWorkOrderNegateNo from WorkOrderNegate";
            using (SqlConnection conn = new SqlConnection(this.connectionString))
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(Qry, conn);
        ...
        public int getNextAdjustmentNo()
        {
            int AdjustmentNo = 0;
            try
            {
                string qry = "SELECT IsNull(MAX(AdjNo),0)+1 FROM  InventoryAdjustment";
                ...
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    ...
                    sqlCon.Close();
                    if (Convert.ToInt32(xResult) > 0)
                        AdjustmentNo = Convert.ToInt32(xResult);
                }
            }
            catch (Exception ex) { throw ex; }
            return AdjustmentNo;
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = "SELECT * FROM [VendorPaymentS] " +
            "Where PaymentID =" + PaymentID;
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            try
        ...
        {
            int NextAutoNo = 0; //
            String Qry = " Select IsNull(max(BillID),0) + 1 as MaxBillID from VendorBill";
            using (SqlConnection conn = new SqlConnection(this.connectionString))
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(Qry, conn);
        ...
        public DataTable GetCustomerRecieptForVoucher(int InvoiceID)
        {
            DataTable dt = new DataTable();
            try
            {
                string qry = "SELECT CR.ID,CR.InvoiceNo,CR.WOID,CR.TrnsDate," +
                             "(Select AccName from Account where ID=4) AS Account," +
                             "(Select top(1)ID from AccountVoucherTypes) as vTypeID,WO.IsCustomerOrder,WO.Status," +
                             "WO.PartsPrice,WO.LaborPrice,WO.OtherPrice,WO.FET,WO.Taxable,WO.Tax,WO.Discount,WO.TotalProfit," +
                             "CR.ChgOnAccount,CR.PayByCash,CR.PaybyCheck,CR.PayByVisa,CR.PayByAMEX,CR.PayByATM,CR.PayByDSCVR,CR.PayByMC,CR.TotalReceivedAmount," +
                             "CR.TrnsNotes,CR.IsLocked from CustomerReceipt CR INNER JOIN WorkOrder WO ON CR.WOID=WO.ID " +
                             "WHERE CR.InvoiceNo=" + InvoiceID + "";
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    sqlCon.Open();
                    SqlCommand sqlCmd = new SqlCommand(qry, sqlCon);
                ...
                if(dt.Rows.Count<1)
                {
                    string qry2 = "SELECT CR.ID,CR.InvoiceNo,CR.WOID,CR.TrnsDate,(Select AccName from Account where ID=4) AS Account," +
                                 " (Select top(1)ID from AccountVoucherTypes) as vTypeID,0 as IsCustomerOrder,0 as Status,0.00 as PartsPrice,0.00 as LaborPrice,0.00 as OtherPrice,0.00 as FET,0.00 as Taxable,0.00 as Tax,0.00 as Discount,0.00 as TotalProfit, CR.ChgOnAccount,CR.PayByCash,CR.PaybyCheck,CR.PayByVisa,"+
                                 " CR.PayByAMEX,CR.PayByATM,CR.PayByDSCVR,CR.PayByMC,CR.TotalReceivedAmount,CR.TrnsNotes,CR.IsLocked from CustomerReceipt CR"+
                                 " WHERE CR.InvoiceNo=" + InvoiceID + "";
                    using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                    {
                        sqlCon.Open();
        ...
        public DataTable GetLatestCustomerRecieptDetails(int WOID,int InvoiceID)
        {
            DataTable dt = new DataTable();
            try
            {
                if(WOID!=0)
                {
                    string qry = "SELECT WOD.ID,WOD.MID,WOD.ItemID,WOD.FeeID,WOD.LaborID,WOD.Ctype," +
                             " WOD.Available,WOD.Qty,WOD.Price,WOD.Cost,WOD.Amount,WOD.DiscAmount," +
                             " WOD.FET,WOD.Total,WOD.Profit,WOD.Tax,WOD.MarginPer,WOD.MarginAmount," +
                             " WOD.AddDate,WOD.AddUserID,WOD.Comments" +
                             " from WorkOrder WO inner join WorkOrderDetail WOD on WO.ID=WOD.MID" +
                             " WHERE WOD.MID=" + WOID + " AND WOD.Active=1";
                    using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                    {
                        sqlCon.Open();
                        SqlCommand sqlCmd = new SqlCommand(qry, sqlCon);
                ...
                {
                    //string qry = "SELECT CR.ID,CR.InvoiceNo,CR.WOID,0 as Ctype,null as MID,1 as Qty,CR.TrnsDate,(Select AccName from Account where ID=4) AS Account," +
                    //             " (Select top(1)ID from AccountVoucherTypes) as vTypeID,0 as IsCustomerOrder,0 as Status,0.00 as Price,0.00 as Amount,0.00 as PartsPrice,0.00 as LaborPrice,0.00 as OtherPrice,0.00 as FET,0.00 as Taxable,0.00 as Tax,0.00 as Discount,0.00 as TotalProfit, CR.ChgOnAccount,CR.PayByCash,CR.PaybyCheck,CR.PayByVisa," +
                    //             " CR.PayByAMEX,CR.PayByATM,CR.PayByDSCVR,CR.PayByMC,CR.TotalReceivedAmount,CR.TrnsNotes,CR.IsLocked from CustomerReceipt CR" +
                    //             " WHERE CR.InvoiceNo=" + InvoiceID + "";
                    //using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                    //{
        ...
        public string getNextVoucherNo()
        {
            ...
            try
            {
                string qry = "SELECT isnull(Max([ID]),0) + 1 FROM [dbo].[AccountVoucherDetails]";
                using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
                {
                    sqlCon.Open();
                    SqlCommand sqlCmd = new SqlCommand(qry, sqlCon);
        ...
        public DataTable getTotalInventoryInfo(int ItemID)
        {
            ...
            try
            {
                string Qry = "SELECT [ItemID], SUM([Qty]) [Qty], SUM([QtyOnHand]) [QtyOnHand]" +
                             " FROM [dbo].[ItemStock]" +
                             " WHERE [ItemID] = " + ItemID +
                             " group by [ItemID]";
        ...
        public DataRow getItemInfo(int ItemID)
        {
            DataTable dt = new DataTable();
            try
            {
                string Qry = " Select I.ID, I.ItemSize, I.Catalog, I.Name, I.CatalogCost, S.QtyOnHand  from Item I" +
                             " Left Join ItemStock S On I.ID=S.ItemID " +
                             " Where I.ID = " + ItemID;
        ...
        //----------UserRights----------------------------------------//
        public bool UpdateSaleRepRights(int EmployeeID)
        {
            bool status = false;
            try
            {
                SqlConnection con = new SqlConnection(this.connectionString);
                SqlCommand cmd = new SqlCommand("SalesRepUserRights", con);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable getUserRights(int EmplyeeID)
        {
            DataTable dt = new DataTable();
            string Qry = "Select M.ID, M.FriendlyName, M.Code, Md.ModuleName, M.Notes," +
                    " (Select CanView From UserRights Ur Where  Ur.MenuID = M.ID  and EmployeeID = " + EmplyeeID +
                    " ) as CanView From Menu M Left Join Module Md On  M.ModuleID = Md.ID Where M.Active=1";
        ...
        {
            DataTable dt = new DataTable();
            string Qry = "Select * from Module";

            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dt);
            return dt;
        }
        public void AddUserRights(DataTable dt)
        {
            using (SqlConnection conn = new SqlConnection(this.connectionString))
            using (SqlCommand cmd = new SqlCommand("dbo.AddUserRights", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;

                cmd.Parameters.Add(new SqlParameter("@tbl", dt));
        ...
        public DataTable DeleteUserRights(int EmplyeeID)
        {
            DataTable dt = new DataTable();
            string Qry = "Delete From UserRights Where EmployeeID = " + EmplyeeID;

            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dt);
            return dt;
        }
        public int InsertModule(string ModuleName,bool active)
        {
            int result = 0;
            string Qry = "INSERT INTO Module(ModuleName,Active) VALUES('" + ModuleName + "','" + active + "')";
            using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
            {
                sqlCon.Open();
                SqlCommand sqlCmd = new SqlCommand(Qry, sqlCon);
        ...
        public int UpdateModule(string ModuleName, bool active,int ID)
        {
            int result = 0;
            string Qry = "Update Module set ModuleName='" + ModuleName + "',Active='" + active + "' where ID = "+ID;
            using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
            {
                sqlCon.Open();
                SqlCommand sqlCmd = new SqlCommand(Qry, sqlCon);
        ...
        {
            int result = 0;
            string Qry = "delete from module where ID = " + ID;
            using (SqlConnection sqlCon = new SqlConnection(this.connectionString))
            {
                sqlCon.Open();
                SqlCommand sqlCmd = new SqlCommand(Qry, sqlCon);
        ...
        public DataTable GetMenu(int ModuleID)
        {
            DataTable dt = new DataTable();
            string Qry = "Select * from Menu where ModuleID="+ModuleID;
        ...
        public DataTable getWarehouseInfo(DataTable dt)
        {
            ...
            string Qry = "SELECT [Wh].[CoName] [CoName]" +
                         " ,[Wh].[CoAddress] [CoAddress], [Wh].[CoPhone] [CoPhone], [Co].[CompanyLogo][CompanyLogo]" +
                         //" ,[Ste1].[Name] + '  ' +[Cty1].[Name] [CoAddress1]" +
                         " FROM [dbo].[Warehouse] [Wh] Left Join Company [Co] on [Co].ID=[Wh].CompanyID" +
                         " Where [Wh].ID =" + StaticInfo.WarehouseID;
            //" LEFT JOIN [dbo].[City] [Cty1] ON [wh].[CityID] = [Cty1].[ID]" +
            //" LEFT JOIN [dbo].[State] [Ste1] ON [Wh].[StateID] = [Ste1].[ID]" +
        ...
        public DataTable getAllCompanyInfo()
        {
            DataTable dataTable = new DataTable();
            string Qry = "SELECT *" +
                         " FROM [dbo].[Company] Where Active =1";

            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            return dataTable;
        }
        public DataTable getCompanyInfoByID(int ID)
        {
            DataTable dataTable = new DataTable();
            string Qry = "SELECT *" +
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = "SELECT * FROM Notifications WHERE Displayed = 0";
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            return dataTable;
        }
        ...
        {
            DataTable dataTable = new DataTable();
            string Qry = "SELECT * FROM Notifications order by AddDate desc";
            SqlDataAdapter sDA = new SqlDataAdapter(Qry, this.connectionString);
            sDA.Fill(dataTable);
            return dataTable;
        }
        public void UpdateDisplayedNotifications(int ID)
        {
            try
            {
                string qry = "UPDATE Notifications SET Displayed=1 WHERE ID =" + ID;
        ...
        public void UpdateSeenNotifications(int ID)
        {
            try
            {
                string qry = "UPDATE Notifications SET Seen=1 WHERE ID ="+ID;
    ...
    public sealed class Helper
    {
        ...
        {
            //return "Data Source=.\\SQLEXPRESS;Initial Catalog=AutoVault-K;User ID=sa;Password=abc@123";
            return @"Server=DESKTOP-3HIBH49;Database=AutoVault-K;Trusted_Connection=True;MultipleActiveResultSets=true; integrated security = true;";
            //return "Data Source=DESKTOP-P81FS32;Initial Catalog=AutoVault-K; User ID=sa;Password=abc@123";
            //return "Data Source=DESKTOP-P81FS32;Initial Catalog=AutoVault-K; User ID=sa;Password=abc@123";

            //return "Data Source=DESKTOP-HDS0179;Initial Catalog=TTS; User ID=sa;Password=abc@123";
            //return @"Data Source=ESS\SQLEXPRESS;Initial Catalog=AutoVault; User ID=sa;Password=abc@123";

            //Live db Credentials
            //return "Data Source=5.189.152.96,1433;Initial Catalog=SouthMountain_Tires;User ID=dynamict_AutoUser; Password=ESS@4646";
            //return "Data Source=5.189.152.96,1433;Initial Catalog=El_Caborca;User ID=dynamict_AutoUser; Password=ESS@4646";
            //return "Data Source=5.189.152.96,1433;Initial Catalog=Client_1;User ID=dynamict_AutoUser; Password=ESS@4646";
            //return "Data Source=5.189.152.96,1433;Initial Catalog=Client_5;User ID=dynamict_AutoUser; Password=ESS@4646";
            //return "Data Source=5.189.152.96,1433;Initial Catalog=totaltire_KS;User ID=dynamict_AutoUser; Password=ESS@4646";
            //return "Data Source=5.189.152.96,1433;Initial Catalog=Chapparo;User ID=dynamict_AutoUser; Password=ESS@4646";
            //return "Data Source=5.189.152.96,1433;Initial Catalog=TTS;User ID=dynamict_AutoUser; Password =ESS@4646";
            //return "Data Source=5.189.152.96,1433;Initial Catalog=Chinos_Tires;User ID=dynamict_AutoUser; Password =ESS@4646";
            //"Data Source=5.189.152.96,1433;Initial Catalog=FamousTireAndWheel;User ID=dynamict_AutoUser; Password =ESS@4646";
